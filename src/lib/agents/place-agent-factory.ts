...
function assertClaimSafety(claim: string) {
  const unsupportedInferences = ["趣味性有保障", "趣味性可由高评分", "体验差", "文化价值低", "评分支撑趣味"];
...
export function createPlaceAgent(
  ...
  const identity = `你代表地点“${factPack.name}”，目标是基于证据说服用户，但必须诚实。只可使用给定 FactPack；事实性论点必须引用 evidenceIds，并且只能逐字陈述 category、distanceMeters、route.walking/driving.durationMinutes、weather.weather/temperatureC、locationLabel、rating 及其对应 evidence。CATEGORY 仅可说活动类型与用户偏好可能匹配；RATING 仅可说公开评分数值或相对高低；WEATHER 仅可说天气和对公园/风景名胜等户外活动舒适度的可能影响。永远不得由这些信息推导“有趣/趣味性有保障/体验好坏/文化价值/室内设施”等未提供事实。不得补充设施、展览、景观细节、阴凉处、活动内容、营业状态、拥挤度或任何未提供事实；缺失信息必须视为未知。可以解释这些已知事实与用户偏好的取舍。输出简短中文。`;
  ...
  return {
    ...
    async rebuttal(attack: DebateMessage): Promise<RebuttalOutput> {
      const output = await model.invoke(
        RebuttalOutputSchema,
        [
          { role: "system", content: identity },
          {
            role: "user",
            content: `用户偏好：${JSON.stringify(userPreference)}\n你的 FactPack：${evidenceSummary(factPack)}\n针对你的唯一实际攻击：${JSON.stringify(attack)}\n必须令 responseToAttackId 等于该攻击 id，attackerPoiId 等于该攻击 speakerPoiId。先承认对方提出的具体 trade-off，再仅用自己的 FactPack 回应，并重新连接用户偏好；不要复制或复述攻击原文，不得新增事实。`,
          },
        ],
        "place_rebuttal",
      );
