...
const BatchExperienceSchema = z.array(z.object({ poiId: z.string().min(1) }).merge(ExperienceProfileSchema));
...
export async function scorePlaceExperiences(candidates: PlaceCandidate[], model: StructuredModel) {
  ...
  const scored = await model.invoke(BatchExperienceSchema, [
    {
      role: "system",
      content: "你是地点体验评分器。根据地点名称、AMap 类型码与类别，为每个地点输出固定七维 ExperienceProfile。不要推荐、不要解释、不要捏造具体设施或价格。自习室、办事点等以专注/办事为主的地点应为 functional；博物馆、展览馆、可游逛艺术空间应为 exploration。必须为输入的每个 poiId 返回一项。",
    },
    {
      role: "user",
  ...
  const byId = new Map(scored.filter((item) => inputIds.has(item.poiId)).map((item) => [item.poiId, item]));
