...
export const DebateResultSchema = z.object({
  ...
  amapQueryMetrics: z.array(z.object({ label: z.string(), typeCodes: z.string(), rawCount: z.number().int().nonnegative() })),
  preScoringPois: z.array(PlaceCandidateSchema),
  experienceScoringMetrics: z.object({ totalCandidates: z.number().int().nonnegative(), totalChunks: z.number().int().nonnegative(), failedChunks: z.number().int().nonnegative(), fallbackCandidates: z.number().int().nonnegative() }),
