...
const leisureExperience: ExperienceProfile = { activityLevel: 0.6, engagementType: "exploration", socialFit: "either", pace: 0.5, spatial: "mixed", stimulation: 0.5, costTier: "low" };
...
describe("experience compatibility filtering", () => {
  const pool = hardFilterPois([
    candidate({ id: "gym", name: "健身工场", category: "体育休闲服务;运动场馆", typeCode: "080101", experienceProfile: { ...leisureExperience, engagementType: "functional", activityLevel: 0.8 } }),
    candidate({ id: "mall", name: "金鹰购物中心", category: "购物服务;商场", typeCode: "060100", experienceProfile: { ...leisureExperience, engagementType: "consumption" } }),
    candidate({ id: "library", name: "南京图书馆", category: "科教文化服务;图书馆", typeCode: "140400", experienceProfile: { ...leisureExperience, engagementType: "functional", activityLevel: 0.1 } }),
    candidate({ id: "cafe", name: "慢慢来咖啡", category: "餐饮服务;咖啡厅", typeCode: "050500", experienceProfile: { ...leisureExperience, engagementType: "social", activityLevel: 0.2 } }),
    candidate({ id: "park", name: "玄武湖公园", category: "风景名胜;公园广场", typeCode: "110000", experienceProfile: leisureExperience }),
  ]);

  it("excludes functional spaces for an exploration intent regardless of AMap category", () => {
    const result = filterIntentCompatiblePois(pool, leisureExperience);
    expect(result.map((candidate) => candidate.id)).not.toContain("library");
    expect(result.map((candidate) => candidate.id)).toContain("park");
  });
});
